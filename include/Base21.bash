...
# Copyright (C) 2013-2021 Joseph Tingiris (joseph.tingiris@gmail.com)
...
declare -x Base_Source="${BASH_SOURCE}"

declare -x APEX_INCLUDE="${Base_Source}" # deprecated; do not use anymore
declare -x Base_Include="${Base_Source}" # deprecated; do not use anymore
declare -x APEX_SOURCE="${Base_Source}" # deprecated; do not use anymore
...
if [ ! -d "${Base_Dir}" ]; then
    echo
    echo "${Base_Dir} directory not found"
    echo
    exit 1
else
    if [ ! -r "${Base_Dir}" ]; then
        echo
        echo "${Base_Dir} directory not readable"
        echo
        exit 1
    fi
fi
...
export Base_Bash=$(readlink -e $BASH_SOURCE)
export Base_Bash_Dir=$(dirname "$Base_Bash")

Base_Debug_Bash_Dir=${Base_Bash_Dir}
Base_Debug_Bash_Depth=0
while [ "${Base_Debug_Bash_Dir}" != "/" ]; do
    ((Base_Debug_Bash_Depth++))

    Base_Debug_Bash="${Base_Debug_Bash_Dir}/debug-bash/Debug.bash"

    if [ -r "${Base_Debug_Bash}" ]; then
        break
    fi

    Base_Debug_Bash_Dir=${Base_Debug_Bash_Dir%/*}
    if [ ${Base_Debug_Bash_Depth} -ge 10 ]; then
        break
    fi
done

if [ -r "${Base_Debug_Bash}" ]; then
    source "${Base_Debug_Bash}"
fi

if [ "$Base_0" == "" ]; then Base_0=$0; fi
if [ "$Base_0" == "-bash" ]; then Base_0=$Base_Bash; fi

if [ "$Base_Arguments" == "" ]; then Base_Arguments=$@; fi

declare -i Base_Arguments_Count=$#

if [ "$Base_Backup_Dir" == "" ]; then Base_Backup_Dir="/backup"; fi # put this outside ${Base_Dir}

if [ "$Base_Datacenters" == "" ]; then Base_Datacenters="atl dal lon man"; fi

if [ "$Base_Environments" == "" ]; then Base_Environments="dev qa staging production"; fi

if [ "$Base_Hostname" == "" ]; then Base_Hostname=$(hostname -s); fi

if [ "$Base_Pid" == "" ]; then Base_Pid=$BASHPID; fi # BASHPID

if [ "$Base_User" == "" ]; then Base_User="$USER"; fi # USER
if [ "$Base_User" == "" ]; then Base_User="nobody"; fi # USER

if [ "$Base_Logname" == "" ]; then Base_Logname="$LOGNAME"; fi # LOGNAME
if [ "$Base_Logname" == "" ]; then Base_Logname="$(logname 2> /dev/null)"; fi # LOGNAME
if [ "$Base_Logname" == "" ]; then Base_Logname="$Base_User"; fi # LOGNAME
if [ "$Base_Logname" == "" ]; then Base_Logname="nobody"; fi # LOGNAME

if [ "$Base_Usage" == "" ]; then Base_Usage=(); fi

# non-Base named globals
...
if [ "$Machine_Backup_Dir" == "" ]; then Machine_Backup_Dir="$Base_Backup_Dir/machine/$Base_Hostname"; fi
...
if [ "$Who" == "" ]; then Who=$Base_User; fi

if [ "$Who" == "" ]; then Who=$Base_Logname; fi
...
export PATH="${Base_Dir}/sbin:${Base_Dir}/bin:${PATH}"
...
function backupFiles() {
    ...
    if [ "$2" == "" ]; then
        local backup_files_directory=$Base_Backup_Dir
    else
    ...
    for backup_file in ${backup_files}; do
        ...
        if [ -f "${backup_file}" ]; then
            ...
            while [ -f "${backup_file_name}" ] || [ "${backup_file_name}" == "" ]; do
                ...
                backup_file_name="${backup_files_directory}/${backup_file_basename}.${Base_Uniq}.${backup_file_counter}"
            done
...
function optionArguments() {
    ...
    if [ "$1" == "" ]; then
        local input_arguments=($Base_Arguments)
    else
    ...
    if [ ${#Base_Usage} -gt 0 ]; then
        Base_Usage+=("")
    fi

    Base_Usage+=("-D | --debug [level]           = display debug messages (less than) [level]")
    Base_Usage+=("-H | --help                    = display this message")
    Base_Usage+=("-V | --version                 = display version")
    Base_Usage+=("")
...
function question() {
    ...
    if [ $Yes_Flag -eq 0 ]; then
        ...
        read Y_N_Q
        echo
        if [ "${Y_N_Q:0:1}" == "q" ]; then baseFinish 1; fi
        if [ "${Y_N_Q:0:1}" == "y" ]; then
            Question_Flag=0
        fi
        Y_N_Q=""
...
function systemLog() {
    ...
    if [ ${#log_message} -gt 0 ]; then
        ...
        #log_message=${log_message//'/\\'}
        # TODO fewer pipes
        echo "${log_message}" | sed -e '/"/s///g' -e "/'/s//\\\'/g" | xargs logger -t "$(basename $Base_0) : $Who : $Who_Ip : $Base_Logname : $Pwd " --
    fi
...
function upgrade() {
    ...
    if [ $upgraded -eq 1 ]; then
        if [ -x "$Base_0" ]; then
            echo "restarting $Base_0 $Base_Arguments ..."
            sync
            sleep 1
            exec $Base_0 $Base_Arguments
            baseFinish 1
        fi
    fi
}
...
function usage() {
    ...
    local base_option base_option_help base_required base_usage base_usage_arguments

    if [ ${#Base_Required} -gt 0 ]; then
        for base_required in ${Base_Required[@]}; do
            base_usage_arguments+="<${base_required}> "
        done
    fi

    if [ ${#Base_Usage} -gt 0 ]; then
        base_usage_arguments+="[options] "
    fi

    base_usage_arguments=${base_usage_arguments##+([[:space:]])} # remove leading whitespaces
    base_usage_arguments=${base_usage_arguments%%+([[:space:]])} # remove trailing whitespaces

    printf "usage: ${Base_0} ${base_usage_arguments}\n\n"

    if [ ${#Base_Usage} -gt 0 ]; then
        printf "options:\n\n"
        for base_usage in "${Base_Usage[@]}"; do
            base_option=${base_usage%%=*}
            base_option=${base_option##+([[:space:]])}
            base_option=${base_option%%+([[:space:]])}

            base_option_help=${base_usage#*=}
            base_option_help=${base_option_help##+([[:space:]])}
            base_option_help=${base_option_help%%+([[:space:]])}

            if [ "${base_option}" == "" ] && [ "${base_option_help}" == "" ]; then
                printf "\n"
                continue
            fi
...
Base_Dependencies+=(awk)
Base_Dependencies+=(basename)
Base_Dependencies+=(cut)
Base_Dependencies+=(date)
Base_Dependencies+=(dirname)
Base_Dependencies+=(egrep)
Base_Dependencies+=(find)
Base_Dependencies+=(grep)
Base_Dependencies+=(head)
Base_Dependencies+=(host)
Base_Dependencies+=(hostname)
Base_Dependencies+=(ip)
Base_Dependencies+=(printf)
Base_Dependencies+=(pwd)
Base_Dependencies+=(readlink)
Base_Dependencies+=(sed)
Base_Dependencies+=(sort)
Base_Dependencies+=(stat)
Base_Dependencies+=(tail)
Base_Dependencies+=(uniq)
Base_Dependencies+=(uuidgen)
Base_Dependencies+=(who)

dependency ${Base_Dependencies[@]}

# Global_Variables (that require dependencies)

if [ "$Base_Account_Dir" == "" ]; then
    Base_Account_Dir="${Base_Dir}/account"
    if [ ! -d "$Base_Account_Dir" ]; then
        mkdir -p "$Base_Account_Dir"
        if [ $? -ne 0 ]; then
            aborting "failed to create account directory $Base_Account_Dir" 4
        fi
    fi
fi

if [ "$Base_Account" == "" ]; then
    Base_Account=$(realpath $(pwd) | awk -F\/ '{print $4}')
    if [ ! -d "$Base_Account_Dir/$Base_Account" ]; then
        Base_Account=""
    fi
fi
...
Base_0_Dir=$(dirname $Base_0)
Base_Dir_0=$Base_0_Dir # deprecated; do not use anymore
...
if [ "$Base_Domain_Name" == "" ]; then
    if [ -r "/etc/domain_name" ]; then
        Base_Domain_Name=$(cat "/etc/domain_name")
    else
        if [ -r "${Base_Dir}/etc/domain_name" ]; then
            Base_Domain_Name=$(cat "${Base_Dir}/etc/domain_name")
        else
...
if [ "$Base_Uniq" == "" ]; then Base_Uniq=$(date +%Y%m%d)-$(uuidgen); fi

if [ "$Tmp_File" == "" ]; then Tmp_File="/tmp/$(basename $Base_0).${Base_Uniq}.tmp"; fi

if [ "$Machine_Class" == "" ]; then Machine_Class=$(echo $Base_Hostname | awk -F\. '{print $1}' | sed -e '/[0-9]/s///g'); fi

if [ "$Machine_Environment" == "" ]; then declare -l Machine_Environment=$(echo $Base_Hostname | awk -F\. '{print $1}' | awk -F- '{print $2}'); fi
...
if [ "$Machine_Name" == "" ]; then Machine_Name=$Base_Hostname; fi

#
# Gobal_Variable (validation)
#

# validate Base_Hostname
if [ "$Base_Hostname" == "" ]; then
    aborting "Base_Hostname is empty"
fi
